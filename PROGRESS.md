...
- 流程固定三段：**检测（只读）→ 决策（回一个数字）→ 执行（确认后写入）**
- 只配置**已启用**模型（在用 + 图像/PDF 这类旁路在用）；未在用不动
- 只做窗口；`maxTokens` 等其它参数、压缩阈值**一律不碰**
- **不留痕**：不写审计、不记录修改历史（仅保留改动前的配置记录供一步回退）
...
- 2026-09-14 13:36：**v1.0.1 扫描结果 = 仍 suspicious，但收窄到一条自相矛盾** → 出 **v1.0.2**
  ...
  - 根因：权限表「绝不写 = 不写任何文件」与「会话状态可编辑 + 先做副本」互相打架
  - **v1.0.2 修改**：① **删除"改会话存储"能力** —— 会话状态行改为「不处理、不修改任何会话存储；只解释显示滞后、由用户自行处置」② 执行第 5 步同步改为「不处理」（不改动即无失败面）③「绝不写」精确化：除窗口字段外不写任何配置项、不创建定时任务、**不写任何本地文件（含不生成备份/审计文件）** ④ 回退记录明确为「**仅本次会话内**记录旧值，不落盘、不写文件」⑤ README 中英 / DESIGN / INTERACTION 同步（去掉"备份/旧值记录"字样）⑥ `docs/EVAL-2026-09-14.md` 移入 `docs/archive/`（历史记录，避免旧版行为描述被当作现行能力）⑦ hero.svg 版本号 → v1.0.2
- 2026-09-14 12:5x：**本地全局安装完成**（用户要求「本地全局安装一下，我来测试」）
  - 启用方式：全局技能目录 + 各 agent 的技能清单（本机细节不在此记录）
  - 关键发现：技能启用采用**白名单制**（全局技能目录 + 各 agent 的技能清单，两者缺一不可；具体清单不在此记录）
  - 执行：以 \config.patch\ 追加启用（保留原有清单，不覆盖）
