...
| **绝不写** | 除上表唯一的窗口字段外，**不写任何配置项**（`maxTokens`、压缩阈值、`agents` / `tools` / `channels` / `plugins` 等）；**不创建定时任务**；**不写任何本地文件**（无脚本、无数据文件、**不生成备份文件或审计文件**） |
...
| 5 | **会话状态（不处理）**：若用户反馈状态显示滞后，只解释"这是显示缓存现象、实际已生效"，并交由用户自行处置；**本技能不修改任何会话存储、不生成备份文件** | 不适用（不改动即无失败面） |
