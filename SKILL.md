...
| **绝不写** | `maxTokens`、压缩阈值、`agents` / `tools` / `channels` / `plugins` 等任何其它配置；不创建定时任务；不写任何文件（无脚本） |
| **联网** | 仅"取数"（读厂商官方来源）；**不上传任何本地数据**、不外发配置内容 |
| **重载 / 重启** | **默认只提示**（说明"需要重载或重启才会刷新运行态"并给出建议命令），**不擅自执行**；仅在用户明确要求时代为操作 |
| **会话状态** | **默认不动**。若用户反馈状态显示滞后并要求处理，范围严格限定为「**仅当前会话**的 `contextTokens` 字段」：先备份该会话存储文件、逐条说明将改动的内容，经用户确认后执行 |
| **回退记录** | 写入前记录**本次将被修改字段的旧值**（一次性、覆盖式、不累积、不编号）→ 仅供一步回退。**不写审计文件、不建历史清单**（设计取舍：动作最小、不留痕）；用户如需留痕，可要求输出「本次变更摘要」自行保存 |
...
| 5 | **会话状态（默认不动）**：若用户反馈状态显示滞后，可提议处理 —— 范围严格限定为「**仅当前会话**的 `contextTokens` 字段」：先备份会话存储文件、说明将改动的内容，经用户确认后执行 | 失败 → 标注"显示滞后"，不影响实际生效 |
